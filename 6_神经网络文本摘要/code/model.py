"""该文件包含为序列到序列模型构建和运行tensorflow graph的代码"""
...
class SummarizationModel(object):
  """表示用于文本摘要的从序列到序列模型的类. 同时支持baseline mode, pointer-generator mode和 coverage"""
  ...
  def _make_feed_dict(self, batch, just_enc=False):
    """创建feed字典，将批处理的部分映射到适当的占位符.
    Args:
      batch: Batch对象
      just_enc: Boolean. 如果true，只提供encoder所需的部分
    """
    feed_dict = {}
  ...
  def _add_encoder(self, encoder_inputs, seq_len):
    ...
    with tf.variable_scope('encoder'):
      ...
      encoder_outputs = tf.concat(axis=2, values=encoder_outputs) # concatenate向前和向后的状态
  ...
  def _add_decoder(self, inputs):
    """向图中添加注意力解码器。在train或eval模式下，只需调用此函数一次，就可以得到所有步骤的输出。在解码(波束搜索)模式中，对每个解码器步骤调用一次。
    Args:
      inputs: 解码器的输入 (word embeddings). A list of tensors shape (batch_size, emb_dim)
    Returns:
  ...
  def _calc_final_dist(self, vocab_dists, attn_dists):
    """计算最终的分布, for the pointer-generator model
    Args:
